...
const baseUrl = import.meta.env.VITE_ADMIN_ORDER_APP_BASE_URL;
...
export const getOrderDetailsForAdmin = createAsyncThunk(
  ...
  async ({ id }) => {
    const response = await axios.get(`${baseUrl}/orders/details/${id}`, {});
    return response.data;
  }
);
...
export const updateOrderStatus = createAsyncThunk(
  ...
  async ({ id, orderStatus }) => {
    const response = await axios.put(`${baseUrl}/orders/update/${id}`, {
      orderStatus,
    });
    return response.data;
  }
